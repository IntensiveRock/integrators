;;; Define the particular numerical integrators.
...
(defmethod ode-step ((integ forward-euler) problem y_n t_n)
  ...
  (pl:compute (pl:lazy #'+ (pl:lazy-array y_n) (pl:lazy #'* (pl:lazy-array (funcall (slot-value problem 'functional-form) y_n t_n)) (slot-value problem 't-step)))))

(defmethod ode-step ((integ heun) problem y_n t_n)
  "Heun numerical integration scheme."
  (let ((t-step (slot-value problem 't-step))
	(y_n (pl:lazy-array y_n))
	(k1 0)
	(k2 0)
	(k3 0))
    (setq k1 (pl:lazy-array (funcall (slot-value problem 'functional-form) y_n t_n)))
    (setq k2 (pl:lazy-array (pl:lazy #'+ y_n (pl:lazy #'* k1 t-step))))
    (setq k3 (pl:lazy-array (funcall (slot-value problem 'functional-form) k1 (+ t_n t-step))))
...
(defmethod ode-step ((integ rk4) problem y_n t_n)
  ...
  (let ((t-step (slot-value problem 't-step))
	...
	(k4 0))
    (setq k1 (pl:lazy-array (funcall (slot-value problem 'functional-form) y_n t_n)))
    (setq k2 (pl:lazy-array (funcall (slot-value problem 'functional-form)
				     (pl:lazy #'+ y_n (pl:lazy #'* k1 (/ t-step 2))) (+ t_n (/ t-step 2)))))
    (setq k3 (pl:lazy-array (funcall (slot-value problem 'functional-form)
				     (pl:lazy #'+ y_n (pl:lazy #'* k2 (/ t-step 2))) (+ t_n (/ t-step 2)))))
    (setq k4 (pl:lazy-array (funcall (slot-value problem 'functional-form)
...
(defun solve (problem solver)
  (let ((times (loop for n from (slot-value problem 't-step) below (slot-value problem 't-max) by  (slot-value problem 't-step) collect n))
	(y_n (slot-value problem 'init-cond))
	(sol (list (slot-value problem 'init-cond))))
